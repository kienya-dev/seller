(() => {
    ...
    document.addEventListener("DOMContentLoaded", (() => {
        ...
        isWebp();
        const initSliders = () => {
            if (document.querySelector("[data-slider-news]")) new core("[data-slider-news]", {
                slidesPerView: 3,
                spaceBetween: 16,
                speed: 400,
                loop: true
            });
            if (document.querySelector("[data-slider-info]")) new core("[data-slider-info]", {
            ...
            if (document.querySelector("[data-slider-rating]")) new core("[data-slider-rating]", {
                modules: [ Navigation ],
                slidesPerView: 3,
                slidesPerGroup: 3,
                spaceBetween: 4,
            ...
            if (document.querySelector("[data-slider-communications]")) new core("[data-slider-communications]", {
                modules: [ Navigation ],
                slidesPerView: 3,
                slidesPerGroup: 3,
                spaceBetween: 4,
            ...
            if (document.querySelector("[data-slider-fbs]")) new core("[data-slider-fbs]", {
                modules: [ Navigation ],
                slidesPerView: 3,
                slidesPerGroup: 3,
                spaceBetween: 4,
            ...
            if (document.querySelector("[data-slider-fbo]")) new core("[data-slider-fbo]", {
                modules: [ Navigation ],
                slidesPerView: 3,
                slidesPerGroup: 3,
                spaceBetween: 4,
        ...
        const showHistorySales = (salesHistory, what) => {
            ...
            chartCounters.forEach((counter => {
                ...
                let res = 0;
                let total = 0;
                if (counter.getAttribute("data-chart-counter") === "order") total = salesHistory.reduce(((acc, sale) => {
                    res += what === "рублях" ? sale.order.sum : sale.order.total;
                    return acc += what === "рублях" ? sale.order.total : sale.order.sum;
                }), 0); else total = salesHistory.reduce(((acc, sale) => {
        ...
        const showDateInterval = salesHistory => {
            ...
            if (monthFrom === monthTo) chartIntervals.forEach((interval => {
                interval.innerHTML = parseInt(dateFrom) ? `${parseInt(dateFrom)} - ${dateTo.split(",")[0]}` : "";
            })); else chartIntervals.forEach((interval => {
        ...
        const newChart = (what = "рублях", interval = "14") => {
            ...
            if (interval === "неделя") {
                ...
                if (thisDate.getDay()) salesHistory.unshift({
                    date: {
                        abbreviated: "",
                        full: ""
                    },
                    order: chartData[chartData.length - (thisDay + 1)].order,
                    delivered: chartData[chartData.length - (thisDay + 1)].delivered
                }); else salesHistory.unshift({
                    date: {
                        abbreviated: "",
                        full: ""
                    },
                    order: chartData[chartData.length - 8].order,
                    delivered: chartData[chartData.length - 8].delivered
                });
            } else {
                salesHistory = chartData.slice(chartData.length - interval, chartData.length);
                if (+interval < 28) salesHistory.unshift({
                    date: {
                        abbreviated: "",
                        full: ""
                    },
                    order: chartData[chartData.length - (+interval + 1)].order,
                    delivered: chartData[chartData.length - (+interval + 1)].delivered
                }); else {
            ...
            const rebuildChart = () => {
                ...
                for (let i = 0; i < dataChartOrders.length; i++) {
                    ...
                    let controlPointX2 = (prevX + x) / 2;
                    let controlPointY2 = y;
                    if (i < dataChartOrders.length - 1) context.bezierCurveTo(controlPointX1, controlPointY1, controlPointX2, controlPointY2, x, y);
                }
                context.setLineDash([ 0, 0 ]);
                context.lineWidth = 2.25;
                ...
                const getMaxTotal = () => salesHistory.map((item => item.order.total)).sort(((a, b) => b - a))[0];
                ...
                if (salesHistory.length < 18) for (let i = 0; i < salesHistory.length; i++) {
                    let x = i * pointWidth + 40;
                    canvas.height, dataChartOrders[i];
                    context.font = "12px Arial";
                    context.fillStyle = "#8d95a5";
                    context.textAlign = "right";
                    context.fillText(salesHistory[i].date.abbreviated, x, canvas.height - 18);
                } else for (let i = 0; i < salesHistory.length; i++) {
                    let x;
                    canvas.height, dataChartOrders[i];
                    if (i % 2 === 0) {
                        x = i * pointWidth + 40;
            ...
            const searchPoint = searchNum => points.find((it => Math.abs(it - searchNum) === Math.min(...points.map((it => Math.abs(it - searchNum))))));
            ...
            canvas1.addEventListener("mousemove", (event => {
                ...
                if (activeNumPoint === 0 || activeNumPoint === points.length - 1) charWindow.style.display = "none";
            }));
        ...
        const toggleActiveIcon = (button, icons) => {
            icons.forEach((icon => {
                ...
                if (icon.closest("button") === button) icon.classList.add("head-menu__select_active");
            }));
        };
        const hideHeadPopup = () => {
        ...
        const showHeadPopup = button => {
            ...
            if (button.getAttribute("data-head-menu") === "bonus") buttons.forEach(((button, idx) => button.addEventListener("click", (() => {
                changeBonus(parent, button, idx);
            }))));
            if (button.getAttribute("data-head-menu") === "sales") buttons.forEach(((button, idx) => {
        ...
        document.addEventListener("click", (e => {
            if (e.target.closest("button") !== activeHeadPopup) hideHeadPopup();
        }));
        ...
        if (productsPage) {
            ...
            const calcTableCardPosition = () => {
                if (activeProductsMenu && activeProductsMenu.getAttribute("data-products-menu") === "table-menu") {
                    if (productsBox.getBoundingClientRect().right - button.getBoundingClientRect().left < activeProductsMenu.getBoundingClientRect().width) activeProductsMenu.style.left = productsBox.getBoundingClientRect().right - activeProductsMenu.getBoundingClientRect().width + "px"; else if (button.getBoundingClientRect().left < 24) activeProductsMenu.style.left = productsBox.getBoundingClientRect().left + "px"; else activeProductsMenu.style.left = button.getBoundingClientRect().left + "px";
                    if (window.innerHeight - button.getBoundingClientRect().bottom < activeProductsMenu.getBoundingClientRect().height + 20) activeProductsMenu.style.top = button.getBoundingClientRect().top - activeProductsMenu.getBoundingClientRect().height + "px"; else activeProductsMenu.style.top = button.getBoundingClientRect().bottom + "px";
                    if (activeProductsMenu.getBoundingClientRect().top < 24) activeProductsMenu.style.top = 24 + "px";
                    if (window.innerHeight - activeProductsMenu.getBoundingClientRect().bottom < 24) activeProductsMenu.style.top = window.innerHeight - activeProductsMenu.getBoundingClientRect().height - 24 + "px";
                }
            };
            const toggleMenu = () => {
                productsMenu.forEach((item => {
                    if (activeProductsMenu !== item) item.classList.remove("active");
                }));
            ...
            const getProductsBox = productsBox => {
                productsBox.addEventListener("scroll", (e => {
                    ...
                    tableFirstCells.forEach((cell => {
                        if (e.target.scrollLeft > 5) cell.classList.add("products-table__cell_highlight"); else cell.classList.remove("products-table__cell_highlight");
                    }));
                    tableLastCells.forEach((cell => {
                        if (e.target.scrollWidth - e.target.scrollLeft - e.target.clientWidth > 5) cell.classList.add("products-table__cell_highlight"); else cell.classList.remove("products-table__cell_highlight");
                    }));
                }));
            };
            ...
            const toggleButton = () => {
                ...
                if (button.classList.contains("products-table__cell_last")) button.classList.toggle("products-table__cell_focus");
            };
            productsButtons.forEach((item => {
            ...
            productsIndexs.forEach((index => {
                ...
                index.addEventListener("mouseout", (() => {
                    if (popupData) popupData.classList.remove("active");
                }));
            }));
                    ...
                    dropdown.addEventListener("click", (event => {
                        ...
                        if (event.currentTarget.closest("li") === parent || event.target === dropdown) {
                            ...
                            submenu.forEach((menu => {
                                if (menu.parentNode === parent) menu.classList.toggle("products-category__sub_active");
                            }));
                        }
                    }));
                }));
            ...
            const getSelectCategoryProducts = activeTab => {
                ...
                productsCatalogCheckboxes.forEach((checkbox => {
                    checkbox.addEventListener("change", (e => {
                        ...
                        if (e.currentTarget.checked) if (parentList.classList.contains("products-category__sub")) {
                            mainCheckbox.classList.add("products-checkbox__custom_parent");
                            checkboxes.forEach((item => {
                                if (item !== e.currentTarget && !item.checked) item.checked = true;
                            }));
                            ...
                            if (parentList.classList.contains("products-category__sub")) {
                                ...
                                checkboxes.forEach((item => {
                                    if (item !== e.currentTarget && item.checked) item.checked = false;
                                }));
                            } else {
                                mainCheckbox.classList.remove("products-checkbox__custom_parent");
                                checkboxes.forEach((item => {
                    ...
                    checkbox.addEventListener("change", (e => {
                        if (e.currentTarget.checked) {
                            ...
                            if (activeBox === "gallery") {
                                const images = activeTab.querySelectorAll(".products-table__image");
                                images.forEach((image => {
                                    if (!image.hasAttribute("data-products-avatar")) image.classList.add("products-table__image_hide");
                                    if (image.classList.contains("products-table__image_none")) image.classList.remove("products-table__image_hide");
                                }));
                            }
                            if (activeBox === "colors") {
